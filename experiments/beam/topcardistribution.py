...
def plot(logFile):
    ...
    for label in possiblesLabels :
        ...
        support = rules.iloc[0][names[4]]+rules.iloc[0][names[6]]
        count = len(rules)
        qualityMean = np.amax(rules[names[3]])
        if qualityMean > maximum : maximum = qualityMean
        qualityStd = np.std(rules[names[3]])
        labels.append(label)
